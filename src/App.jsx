...
const colors = {
  ink: "#070C16",       // background nyeusi-buluu (header/nav)
  bg: "#05080F",        // background ya nje kabisa
  surface: "#111A2B",   // "kadi" (posts, inputs, bubbles) - giza kidogo zaidi ya ink
  cream: "#EAF6FF",     // maandishi/icons meupe-baridi kwenye background giza
  sunset: "#22D3EE",    // neon cyan-blue - kitendo kikuu (CTA)
  gold: "#3BFFB0",      // neon green - lafudhi ya pili
  forest: "#12E8A8",    // neon teal-green - video/success
  charcoal: "#DCE6F2",  // maandishi mepesi ndani ya kadi za giza
  muted: "#7E8FA8",     // maandishi ya pili/ya kufifia
  line: "#1E2A3F",       // mipaka myeusi myembamba
};
const NEON_GRADIENT = "linear-gradient(90deg, #22D3EE, #3BFFB0)";
...
function ReelsScreen({ uid, token, t, onToggleLike, onOpenComments, onClose }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const rows = await apiGet("posts?type=eq.video&select=*,users(name,handle,avatar_url)&order=created_at.desc&limit=30", token);
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-6">
          {mode === "translate" && (
            <select value={targetLang} onChange={(e) => setTargetLang(e.target.value)}
              className="w-full px-3 py-2 rounded-xl text-sm outline-none mb-3" style={{ backgroundColor: colors.surface, color: colors.cream }}>
              {LANGUAGES.map((l) => (
...
function ExploreScreen({ uid, token, t, onToggleLike, onOpenComments }) {
  ...
  const load = async () => {
    ...
    try {
      if (active === "trending") {
        const rows = await apiGet("posts?select=*,users(name,handle,avatar_url)&order=created_at.desc&limit=50", token);
        ...
        const rows = await apiGet(`posts?select=*,users(name,handle,avatar_url)&or=(${filter})&order=created_at.desc&limit=30`, token);
        setPosts(await withCounts(rows));
      } else {
        const rows = await apiGet(`posts?select=*,users(name,handle,avatar_url)&category=eq.${active}&order=created_at.desc&limit=30`, token);
        setPosts(await withCounts(rows));
      }
    } catch (e) {
      setPosts([]);
        ...
        {view === "sell" && (
          <div className="flex-1 overflow-y-auto px-4 space-y-3">
            ...
            <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}
              className="w-full px-3 py-2 rounded-xl text-sm outline-none" style={{ backgroundColor: colors.surface, color: colors.cream }}>
...
function ProfileScreen({ profile, myPosts, onSignOut, t, onOpenCircles }) {
  return (
  ...
  const loadFeed = async () => {
    ...
    try {
      const data = await apiGet(
        "posts?select=*,users(name,handle,avatar_url)&order=created_at.desc&limit=50", token
      );
      const likes = await apiGet(`likes?user_id=eq.${uid}&select=post_id`, token);
      const likedIds = new Set(likes.map((l) => l.post_id));
      const withCounts = await Promise.all(
    ...
    <div className="min-h-screen w-full flex justify-center" style={{ backgroundColor: colors.bg, fontFamily: "'Inter', sans-serif" }} dir={isRtl ? "rtl" : "ltr"}>
      ...
      <div className="w-full max-w-md min-h-screen relative flex flex-col" style={{ backgroundColor: colors.surface }}>
        <div className="sticky top-0 z-20 flex items-center justify-between px-4 py-4" style={{ backgroundColor: colors.ink }}>
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setShowBusiness(true)}><Building2 size={20} color={colors.cream} /></button>
            <button onClick={() => setShowGaming(true)}><Gamepad2 size={20} color={colors.cream} /></button>
            <button onClick={() => setShowLearning(true)}><GraduationCap size={20} color={colors.cream} /></button>
            <button onClick={() => setShowJobs(true)}><Briefcase size={20} color={colors.cream} /></button>
            <button onClick={() => setShowMarketplace(true)}><ShoppingBag size={20} color={colors.cream} /></button>
            <button onClick={() => setLiveMode("list")}><Radio size={20} color={colors.cream} /></button>
            <button onClick={() => setShowAI(true)}><Sparkles size={20} color={colors.cream} /></button>
            <button onClick={() => setShowReels(true)}><Play size={20} color={colors.cream} /></button>
            <button onClick={() => setShowDM(true)}><MessageSquare size={20} color={colors.cream} /></button>
            <select value={lang} onChange={(e) => setLang(e.target.value)} className="text-xs rounded-full px-2 py-1 outline-none"
              style={{ backgroundColor: "rgba(255,255,255,0.1)", color: colors.cream }}>
              {LANGUAGES.map((l) => (
                <option key={l.code} value={l.code} style={{ color: "#1a1a1a" }}>{l.flag}</option>
              ))}
            </select>
            <span className="text-xs" style={{ color: colors.gold }}>{profile?.handle}</span>
          </div>
        </div>
        <div className="h-[2px] w-full sticky top-[60px] z-20" style={{ backgroundImage: NEON_GRADIENT, boxShadow: "0 0 8px 1px rgba(34,211,238,0.6)" }} />
        ...
        {tab === "profile" && <ProfileScreen profile={profile} myPosts={myPosts} onSignOut={handleSignOut} t={t} onOpenCircles={() => setShowCircles(true)} />}
