...
import {
  ...
  LayoutGrid, Settings as SettingsIcon, Palette, Edit3, Flag, Shield, Ban, Megaphone, RefreshCw,
} from "lucide-react";
...
const TRANSLATIONS = {
  en: {
    ...
    editProfile: "Edit Profile", shareProfile: "Share Profile", linkCopied: "Link copied!", bio: "Bio",
    followersLabel: "Followers", followingLabel: "Following", posts: "Posts", save: "Save",
    report: "Report", reportReason: "What's the issue?", reportSubmit: "Submit report", reported: "Reported",
    admin: "Admin", adminPanel: "Admin Panel", totalUsers: "Total users", totalPosts: "Total posts", activeLives: "Active lives",
    reportsTab: "Reports", broadcastTab: "Broadcast", usersTab: "Users", noReports: "No reports.",
    resolve: "Resolve", suspend: "Suspend", unsuspend: "Unsuspend", broadcastPh: "Write announcement to all users...",
    sendBroadcast: "Send to everyone", searchUserPh: "Search user by name/handle...", suspended: "Suspended",
  ...
  sw: {
    ...
    editProfile: "Hariri Wasifu", shareProfile: "Shiriki Wasifu", linkCopied: "Kiungo kimenakiliwa!", bio: "Kuhusu",
    followersLabel: "Wafuasi", followingLabel: "Anaofuata", posts: "Machapisho", save: "Hifadhi",
    report: "Ripoti", reportReason: "Tatizo ni nini?", reportSubmit: "Wasilisha ripoti", reported: "Imeripotiwa",
    admin: "Admin", adminPanel: "Admin Panel", totalUsers: "Watumiaji wote", totalPosts: "Machapisho yote", activeLives: "Live zinazoendelea",
    reportsTab: "Ripoti", broadcastTab: "Tangazo", usersTab: "Watumiaji", noReports: "Hakuna ripoti.",
    resolve: "Kamilisha", suspend: "Simamisha", unsuspend: "Rudisha", broadcastPh: "Andika tangazo kwa watumiaji wote...",
    sendBroadcast: "Tuma kwa wote", searchUserPh: "Tafuta mtumiaji kwa jina/handle...", suspended: "Amesimamishwa",
    accountSuspended: "Akaunti yako imesimamishwa.",
  },
  fr: {
    ...
    <div className="rounded-2xl mb-4 overflow-hidden border" style={{ backgroundColor: colors.surface, borderColor: colors.line }}>
      <div className="flex items-center justify-between px-4 pt-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-semibold text-sm" style={{ color: colors.charcoal }}>{author.name}</p>
    ...
    <div className="fixed inset-0 flex items-end sm:items-center justify-center z-50" style={{ backgroundColor: "rgba(27,42,74,0.55)" }} onClick={onClose}>
      <div className="w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl p-5" style={{ backgroundColor: colors.surface }} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold" style={{ color: colors.charcoal, fontFamily: "'Fraunces', serif" }}>{t("composeTitle")}</h2>
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" style={{ backgroundColor: "rgba(27,42,74,0.55)" }} onClick={onClose}>
      <div className="w-full sm:max-w-md h-[70vh] rounded-t-3xl sm:rounded-3xl flex flex-col" style={{ backgroundColor: colors.surface }} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: colors.line }}>
          <h2 className="text-lg font-bold" style={{ color: colors.charcoal, fontFamily: "'Fraunces', serif" }}>{t("commentsTitle")}</h2>
          <button onClick={onClose}><X size={20} color={colors.cream} /></button>
        </div>
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          {comments.map((c) => (
            <div key={c.id} className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-semibold" style={{ color: colors.charcoal }}>{c.users?.name || "..."}</p>
            ...
            {conversations.map((c) => (
              <button key={c.id} onClick={() => openConversation(c)} className="w-full flex items-center gap-3 px-4 py-3 border-b text-left" style={{ borderColor: colors.line }}>
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold" style={{ color: colors.charcoal }}>{c.other.name}</p>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4">
            ...
            {results.map((u) => (
              <button key={u.id} onClick={() => startConversation(u)} className="w-full flex items-center gap-3 py-3 text-left">
                ...
                <div>
                  <p className="text-sm font-semibold" style={{ color: colors.charcoal }}>{u.name}</p>
...
function ExploreScreen({ uid, token, t, onToggleLike, onOpenComments }) {
  ...
  return (
    <div className="px-4 pb-24 pt-4">
      <h2 className="text-xl font-bold mb-3" style={{ color: colors.charcoal, fontFamily: "'Fraunces', serif" }}>{t("explore")}</h2>
            ...
            <div className="grid grid-cols-2 gap-3 mt-2">
              {products.map((p) => (
                <button key={p.id} onClick={() => { setActiveProduct(p); setView("detail"); }} className="rounded-xl overflow-hidden text-left" style={{ backgroundColor: colors.surface }}>
                  ...
                  <div className="p-2">
                    <p className="text-xs font-semibold truncate" style={{ color: colors.charcoal }}>{p.title}</p>
            ...
            {jobs.map((j) => (
              <button key={j.id} onClick={() => openJob(j)} className="w-full text-left rounded-xl p-4 mb-3" style={{ backgroundColor: colors.surface }}>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="text-sm font-bold" style={{ color: colors.charcoal }}>{j.title}</p>
                </div>
            ...
            {courses.map((c) => (
              <button key={c.id} onClick={() => openCourse(c)} className="w-full text-left rounded-xl p-4 mb-3" style={{ backgroundColor: colors.surface }}>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="text-sm font-bold" style={{ color: colors.charcoal }}>{c.title}</p>
                </div>
          ...
          <div className="flex-1 overflow-y-auto px-4">
            ...
            {lessons.map((l, i) => (
              <div key={l.id} className="rounded-xl p-3 mb-2 flex items-start gap-3" style={{ backgroundColor: colors.surface }}>
                ...
                <div>
                  <p className="text-sm font-semibold" style={{ color: colors.charcoal }}>{i + 1}. {l.title}</p>
        ...
        {view === "menu" && (
          <div className="flex-1 overflow-y-auto px-4">
            <div className="rounded-2xl p-5 mb-5 text-center" style={{ backgroundColor: colors.surface }}>
              ...
              <p className="text-base font-bold mb-3" style={{ color: colors.charcoal, fontFamily: "'Fraunces', serif" }}>{t("quickMath")}</p>
          ...
          <div className="flex-1 overflow-y-auto px-4">
            ...
            {pages.map((p) => (
              <button key={p.id} onClick={() => openPage(p)} className="w-full text-left rounded-xl p-4 mb-3 flex items-center gap-3" style={{ backgroundColor: colors.surface }}>
                ...
                <div>
                  <p className="text-sm font-bold" style={{ color: colors.charcoal }}>{p.name}</p>
...
function MoreMenuScreen({ t, onClose, onOpen }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" style={{ backgroundColor: "rgba(0,0,0,0.6)" }} onClick={onClose}>
      <div className="w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl p-5" style={{ backgroundColor: colors.surface }} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold" style={{ color: colors.charcoal, fontFamily: "'Fraunces', serif" }}>{t("more")}</h2>
          <button onClick={onClose}><X size={20} color={colors.cream} /></button>
        </div>
...
function EditProfileModal({ uid, token, profile, t, onClose, onSaved }) {
  const [name, setName] = useState(profile?.name || "");
  const [bio, setBio] = useState(profile?.bio || "");
  const [saving, setSaving] = useState(false);

  const reroll = () => `https://i.pravatar.cc/150?u=${uid}-${Date.now()}`;
  const [avatar, setAvatar] = useState(profile?.avatar_url || "");

  const save = async () => {
    setSaving(true);
    try {
      const updated = await apiPatch(`users?id=eq.${uid}`, { name, bio, avatar_url: avatar }, token);
      onSaved(updated[0]);
      onClose();
    } catch (e) {}
    setSaving(false);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" style={{ backgroundColor: "rgba(0,0,0,0.55)" }} onClick={onClose}>
      <div className="w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl p-5" style={{ backgroundColor: colors.surface }} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold" style={{ color: colors.charcoal, fontFamily: "'Fraunces', serif" }}>{t("editProfile")}</h2>
          <button onClick={onClose}><X size={20} color={colors.charcoal} /></button>
        </div>
        <div className="flex flex-col items-center mb-4">
          <img src={avatar || `https://i.pravatar.cc/150?u=${uid}`} alt="" className="w-20 h-20 rounded-full object-cover mb-2" style={{ boxShadow: `0 0 0 3px ${colors.gold}` }} />
          <button onClick={() => setAvatar(reroll())} className="text-xs font-semibold flex items-center gap-1" style={{ color: colors.gold }}>
            <RefreshCw size={12} /> {t("editProfile")}
          </button>
        </div>
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder={t("productTitle")}
          className="w-full px-3 py-2 rounded-xl text-sm outline-none mb-3" style={{ backgroundColor: colors.ink, color: colors.cream }} />
        <textarea value={bio} onChange={(e) => setBio(e.target.value)} placeholder={t("bio")} rows={3}
          className="w-full px-3 py-2 rounded-xl text-sm outline-none resize-none mb-4" style={{ backgroundColor: colors.ink, color: colors.cream }} />
        <button onClick={save} disabled={saving} className="w-full py-3 rounded-xl font-semibold text-sm" style={{ backgroundImage: NEON_GRADIENT, color: colors.ink }}>
          {t("save")}
        </button>
      </div>
    </div>
...
function AdminScreen({ uid, token, t, onClose }) {
  const [tab, setTab] = useState("stats");
  const [stats, setStats] = useState({ users: 0, posts: 0, lives: 0 });
  ...
  useEffect(() => {
    (async () => {
      try {
        const [users, posts, lives] = await Promise.all([
          apiGet("users?select=id", token),
          apiGet("posts?select=id", token),
          apiGet("lives?status=eq.active&select=id", token),
        ]);
        setStats({ users: users.length, posts: posts.length, lives: lives.length });
      } catch (e) {}
    })();
    loadReports();
  }, []);
  ...
  const loadReports = async () => {
    try {
      const rows = await apiGet("reports?status=eq.open&select=*&order=created_at.desc", token);
      setReports(rows);
    } catch (e) {}
  };
  ...
  const resolveReport = async (id) => {
    try {
      await apiPatch(`reports?id=eq.${id}`, { status: "resolved" }, token);
  ...
  const sendBroadcast = async () => {
    ...
    try {
      await apiPost("announcements", { admin_id: uid, message: broadcastMsg }, token);
      setBroadcastMsg("");
      setSent(true);
  ...
  const searchUsers = async (q) => {
    ...
    try {
      const rows = await apiGet(`users?or=(name.ilike.*${q}*,handle.ilike.*${q}*)&select=*&limit=15`, token);
      setUserResults(rows);
    } catch (e) {}
  };
  ...
  const toggleSuspend = async (u) => {
    try {
      await apiPatch(`users?id=eq.${u.id}`, { is_suspended: !u.is_suspended }, token);
      setUserResults((prev) => prev.map((r) => r.id === u.id ? { ...r, is_suspended: !r.is_suspended } : r));
    } catch (e) {}
  };

  const TABS = [
    { key: "stats", label: t("adminPanel") },
    { key: "reports", label: t("reportsTab") },
    { key: "broadcast", label: t("broadcastTab") },
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-center" style={{ backgroundColor: colors.ink }}>
      <div className="w-full max-w-md h-full flex flex-col">
        <div className="flex items-center gap-3 px-4 py-4">
          <button onClick={onClose}><ArrowLeft size={20} color={colors.cream} /></button>
          <h2 className="text-lg font-bold" style={{ color: colors.cream, fontFamily: "'Fraunces', serif" }}>{t("adminPanel")}</h2>
        </div>
        <div className="flex gap-2 px-4 pb-3 overflow-x-auto" style={{ scrollbarWidth: "none" }}>
          {TABS.map((tb) => (
            <button key={tb.key} onClick={() => setTab(tb.key)} className="px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap"
              style={{ backgroundImage: tab === tb.key ? NEON_GRADIENT : "none", backgroundColor: tab === tb.key ? "transparent" : colors.surface, color: tab === tb.key ? colors.ink : colors.charcoal }}>
              {tb.label}
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-4">
          {tab === "stats" && (
            <div className="grid grid-cols-3 gap-3">
              <div className="rounded-xl p-4 text-center" style={{ backgroundColor: colors.surface }}>
                <p className="text-2xl font-bold" style={{ color: colors.gold }}>{stats.users}</p>
                <p className="text-xs" style={{ color: colors.muted }}>{t("totalUsers")}</p>
              </div>
              <div className="rounded-xl p-4 text-center" style={{ backgroundColor: colors.surface }}>
                <p className="text-2xl font-bold" style={{ color: colors.gold }}>{stats.posts}</p>
                <p className="text-xs" style={{ color: colors.muted }}>{t("totalPosts")}</p>
              </div>
              <div className="rounded-xl p-4 text-center" style={{ backgroundColor: colors.surface }}>
                <p className="text-2xl font-bold" style={{ color: colors.gold }}>{stats.lives}</p>
                <p className="text-xs" style={{ color: colors.muted }}>{t("activeLives")}</p>
              </div>
            </div>
          )}
          ...
          {tab === "reports" && (
            <div>
              {reports.length === 0 && <p className="text-sm text-center mt-6" style={{ color: colors.muted }}>{t("noReports")}</p>}
              {reports.map((r) => (
                <div key={r.id} className="rounded-xl p-3 mb-2" style={{ backgroundColor: colors.surface }}>
                  <p className="text-xs font-semibold mb-1" style={{ color: colors.gold }}>{r.target_type}</p>
                  <p className="text-sm mb-2" style={{ color: colors.charcoal }}>{r.reason}</p>
                  <button onClick={() => resolveReport(r.id)} className="text-xs font-semibold" style={{ color: colors.forest }}>{t("resolve")}</button>
                </div>
              ))}
            </div>
          )}
          ...
          {tab === "broadcast" && (
            <div>
              <textarea value={broadcastMsg} onChange={(e) => setBroadcastMsg(e.target.value)} placeholder={t("broadcastPh")} rows={4}
                className="w-full px-3 py-2 rounded-xl text-sm outline-none resize-none mb-3" style={{ backgroundColor: colors.surface, color: colors.charcoal }} />
              <button onClick={sendBroadcast} className="w-full py-3 rounded-xl font-semibold text-sm" style={{ backgroundImage: NEON_GRADIENT, color: colors.ink }}>
                <Megaphone size={14} className="inline mr-1" /> {sent ? t("reported") : t("sendBroadcast")}
              </button>
            </div>
          )}
          ...
          {tab === "users" && (
            <div>
              <input value={userQuery} onChange={(e) => searchUsers(e.target.value)} placeholder={t("searchUserPh")}
                className="w-full px-3 py-2 rounded-xl text-sm outline-none mb-3" style={{ backgroundColor: colors.surface, color: colors.charcoal }} />
              {userResults.map((u) => (
                <div key={u.id} className="flex items-center gap-3 py-2">
                  <img src={u.avatar_url || `https://i.pravatar.cc/150?u=${u.id}`} alt="" className="w-9 h-9 rounded-full object-cover" />
                  <div className="flex-1">
                    <p className="text-sm font-semibold" style={{ color: colors.cream }}>{u.name} {u.is_suspended && <span style={{ color: "#E23B3B" }}>({t("suspended")})</span>}</p>
                    <p className="text-xs" style={{ color: colors.muted }}>{u.handle}</p>
                  </div>
                  <button onClick={() => toggleSuspend(u)} className="text-xs font-semibold px-2 py-1 rounded-full" style={{ backgroundColor: u.is_suspended ? colors.forest : "#E23B3B", color: colors.ink }}>
...
function ProfileScreen({ uid, token, profile, myPosts, onSignOut, t, onOpenCircles, onOpenSettings, onOpenEdit, onOpenAdmin }) {
    ...
    (async () => {
      try {
        const [followers, following] = await Promise.all([
          apiGet(`follows?following_id=eq.${uid}&select=follower_id`, token),
          apiGet(`follows?follower_id=eq.${uid}&select=following_id`, token),
        ]);
  ...
  const shareProfile = () => {
    const url = `https://imme.app/${(profile?.handle || "").replace("@", "")}`;
  ...
  return (
    <div className="pb-24">
      <div className="w-full h-32" style={{ backgroundColor: colors.line, backgroundImage: profile?.banner_url ? `url(${profile.banner_url})` : NEON_GRADIENT, backgroundSize: "cover", backgroundPosition: "center" }} />
      <div className="px-4">
        <div className="flex items-center justify-between mt-2 mb-3">
          <h2 className="text-xl font-bold" style={{ color: colors.charcoal, fontFamily: "'Fraunces', serif" }}>{t("profile")}</h2>
          <div className="flex items-center gap-3">
            {profile?.is_admin && <button onClick={onOpenAdmin}><Shield size={19} color={colors.gold} /></button>}
            <button onClick={onOpenSettings}><SettingsIcon size={19} color={colors.charcoal} /></button>
        ...
        <div className="flex items-end gap-4 -mt-10 mb-3">
          <img src={profile?.avatar_url || "https://i.pravatar.cc/150"} alt="" className="w-20 h-20 rounded-full object-cover" style={{ boxShadow: `0 0 0 3px ${colors.surface}` }} />
        </div>
        <p className="font-semibold text-base" style={{ color: colors.charcoal }}>{profile?.name}</p>
        <p className="text-sm mb-2" style={{ color: colors.muted, fontFamily: "'IBM Plex Mono', monospace" }}>{profile?.handle}</p>
        {profile?.bio && <p className="text-sm mb-3" style={{ color: colors.charcoal }}>{profile.bio}</p>}

        <div className="flex items-center gap-5 mb-4">
          <div><span className="font-bold text-sm" style={{ color: colors.charcoal }}>{myPosts.length}</span> <span className="text-xs" style={{ color: colors.muted }}>{t("posts")}</span></div>
          <div><span className="font-bold text-sm" style={{ color: colors.charcoal }}>{counts.followers}</span> <span className="text-xs" style={{ color: colors.muted }}>{t("followersLabel")}</span></div>
          <div><span className="font-bold text-sm" style={{ color: colors.charcoal }}>{counts.following}</span> <span className="text-xs" style={{ color: colors.muted }}>{t("followingLabel")}</span></div>
        </div>

        <div className="flex gap-2 mb-3">
          <button onClick={onOpenEdit} className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-sm font-semibold" style={{ backgroundColor: colors.surface, color: colors.charcoal }}>
            <Edit3 size={14} /> {t("editProfile")}
          </button>
          <button onClick={shareProfile} className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-sm font-semibold" style={{ backgroundColor: colors.surface, color: colors.charcoal }}>
            <Share2 size={14} /> {copied ? t("linkCopied") : t("shareProfile")}
          </button>
        </div>

        <button onClick={onOpenCircles} className="w-full flex items-center gap-2 py-3 px-3 rounded-xl mb-3 border" style={{ borderColor: colors.line }}>
          <Users size={16} color={colors.gold} />
          <span className="text-sm font-semibold" style={{ color: colors.charcoal }}>{t("circles")}</span>
        </button>
        <div className="flex items-center gap-2 mb-3 pb-2 border-b" style={{ borderColor: colors.line }}>
          <Grid3x3 size={16} color={colors.charcoal} />
          <span className="text-sm font-semibold" style={{ color: colors.charcoal }}>{t("myPosts")} ({myPosts.length})</span>
        </div>
        <div className="space-y-3">
          {myPosts.map((p) => (
...
export default function ImMeApp() {
  ...
  if (profile?.is_suspended) {
    return (
      <div className="min-h-screen w-full flex flex-col items-center justify-center px-6" style={{ backgroundColor: colors.ink }}>
        <Ban size={40} color="#E23B3B" className="mb-4" />
        <p className="text-lg font-bold mb-2" style={{ color: colors.cream, fontFamily: "'Fraunces', serif" }}>{t("suspended")}</p>
        <p className="text-sm text-center mb-6" style={{ color: colors.muted }}>{t("accountSuspended")}</p>
        <button onClick={handleSignOut} className="px-6 py-3 rounded-xl font-semibold text-sm" style={{ backgroundColor: colors.surface, color: colors.charcoal }}>
          {t("signOut")}
        </button>
      </div>
    );
  ...
  return (
    <div className="min-h-screen w-full flex justify-center" style={{ backgroundColor: colors.bg, fontFamily: "'Inter', sans-serif" }} dir={isRtl ? "rtl" : "ltr"}>
      ...
      <div className="w-full max-w-md min-h-screen relative flex flex-col" style={{ backgroundColor: colors.surface }}>
        ...
        {tab === "profile" && (
          <ProfileScreen uid={uid} token={token} profile={profile} myPosts={myPosts} onSignOut={handleSignOut} t={t}
            onOpenCircles={() => setShowCircles(true)} onOpenSettings={() => setShowSettings(true)}
            onOpenEdit={() => setShowEditProfile(true)} onOpenAdmin={() => setShowAdmin(true)} />
        )}
        ...
        {showEditProfile && (
          <EditProfileModal uid={uid} token={token} profile={profile} t={t} onClose={() => setShowEditProfile(false)} onSaved={(p) => setProfile(p)} />
        )}
        {showAdmin && <AdminScreen uid={uid} token={token} t={t} onClose={() => setShowAdmin(false)} />}
