...
const THEMES = {
  ...
  daylight: { name: "Classic Light", ink: "#1B2A4A", bg: "#E9E2D0", surface: "#FBF6ED", cream: "#1B2A4A", sunset: "#E8622C", gold: "#D4A72C", forest: "#3F7D5C", charcoal: "#22262E", muted: "#9A8F7A", line: "#EDE6D6" },
};
    ...
    <div className="fixed inset-0 z-50 flex justify-center" style={{ backgroundColor: colors.ink }}>
      <div className="w-full max-w-md h-full flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-4">
          ...
          <select value={lang} onChange={(e) => setLang(e.target.value)}
            className="w-full px-3 py-3 rounded-xl text-sm outline-none mb-6" style={{ backgroundColor: colors.surface, color: colors.cream }}>
            {LANGUAGES.map((l) => (
              <option key={l.code} value={l.code} style={{ color: "#1a1a1a" }}>{l.flag} {l.name}</option>
            ))}
          </select>
